...
class VisualBERTForClassification(nn.Module):
    ...
    def relprop(self, cam, **kwargs):
        for m in reversed(self.classifier._modules.values()):
            cam = m.relprop(cam, **kwargs)
        cam = self.vqa_pooler.relprop(cam, **kwargs)
...
class VisualBERT(BaseModel):
    ...
    def forward(self, sample_list: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        ######## remove padding
        sum_text_mask = sample_list["input_mask"].sum()
        max_text_len = sample_list["input_mask"].shape[-1]
    ...
    def relprop(self, cam, **kwargs):
        return self.model.relprop(cam, **kwargs)
